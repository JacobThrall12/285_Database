...
async function getAllPokemon(parameters = {}) {
    let selectSql = 'SELECT * FROM create_a_team_prototype',
        whereStatements = [],
        orderByStatements = [],
        queryParameters = [];
...
async function insertPokemon(parameters = {}) {
    const insertSql = 'INSERT INTO create_a_team_prototype (name, image_urls, height, weight, primary_type, secondary_type) VALUES(?, ?, ?, ?, ?, ?)'
    let queryParameters = [
        parameters.name,
...
async function updatePokemon(id, parameters = {}) {
    const updateSql = 'UPDATE create_a_team_prototype SET name=?, image_urls=?, height=?, weight=?, primary_type=?, secondary_type=? WHERE id = ?'
    let queryParameters = [
        parameters.name,
        parameters.image_urls,
        parameters.height,
