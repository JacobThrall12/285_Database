...
const teamValidation = [
  check('head_coach', 'Please choose a coach').isIn(['Cynthia','Cyrus','Giovanni','Steven','Gary'])
]
//PART 1 of 5 (SELECT) 
...
app.get('/teams/', upload.none(),
  async (request, response) => {
    let result = {};
    try {
      result = await teams.combineTables(request.query);
    } catch (error) {
      return response
        .status(500) //Error code when something goes wrong with the server
        .json({ message: 'Something went wrong with the server.' });
...
app.post('/teams/', upload.none(), teamValidation, async (request, response) => {
  const errors = validationResult(request)
  if (!errors.isEmpty()) {
    return response
      .status(400)
  ...
  try {
    await teams.insertTeam(request.body);
    return response
      .json({ message: 'OK' });
